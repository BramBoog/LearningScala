object AoC2015Day07 extends App:
        ...
        wires.get(forWire) match
            ...
            case None => 
                instr.head.evaluate(wires) match
                    case None => solve(forWire, wires, instr.tail.appended(instr.head))
                    case Some(x) => solve(forWire, wires + (instr.head.retName -> x), instr.tail)
    ...
    val answer1: Int = solve("a", Map(), instructions)
